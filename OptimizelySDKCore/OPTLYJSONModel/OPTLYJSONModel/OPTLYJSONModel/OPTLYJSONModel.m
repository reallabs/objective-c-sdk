...
static OPTLYJSONValueTransformer* valueTransformer = nil;
static Class OPTLYJSONModelClass = NULL;

#pragma mark - model cache
static OPTLYJSONKeyMapper* globalKeyMapper = nil;

#pragma mark - OPTLYJSONModel implementation
@implementation OPTLYJSONModel
{
...
+(void)load
{
    ...
    dispatch_once(&once, ^{
        ...
        // which are common for ALL OPTLYJSONModel subclasses
        ...
        @autoreleasepool {
            ...
            valueTransformer = [[OPTLYJSONValueTransformer alloc] init];

            // This is quite strange, but I found the test isSubclassOfClass: (line ~291) to fail if using [OPTLYJSONModel class].
            ...
            // Using NSClassFromString instead of [OPTLYJSONModel class], as this was breaking unit tests, see below
            //http://stackoverflow.com/questions/21394919/xcode-5-unit-test-seeing-wrong-class
            OPTLYJSONModelClass = NSClassFromString(NSStringFromClass(self));
        }
    });
}
...
-(instancetype)initWithData:(NSData *)data error:(NSError *__autoreleasing *)err
{
    //check for nil input
    if (!data) {
        if (err) *err = [OPTLYJSONModelError errorInputIsNil];
        return nil;
    }
    //read the json
    OPTLYJSONModelError* initError = nil;
    id obj = [NSJSONSerialization JSONObjectWithData:data
                                             options:kNilOptions
                                               error:&initError];

    if (initError) {
        if (err) *err = [OPTLYJSONModelError errorBadJSON];
        return nil;
    }
...
-(id)initWithString:(NSString*)string error:(OPTLYJSONModelError**)err
{
    OPTLYJSONModelError* initError = nil;
...
-(id)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(OPTLYJSONModelError**)err
{
    //check for nil input
    if (!string) {
        if (err) *err = [OPTLYJSONModelError errorInputIsNil];
...
-(id)initWithDictionary:(NSDictionary*)dict error:(NSError**)err
{
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        if (err) *err = [OPTLYJSONModelError errorInvalidDataWithMessage:@"Attempt to initialize OPTLYJSONModel object using initWithDictionary:error: but the dictionary parameter was not an 'NSDictionary'."];
        return nil;
    }
    ...
    if (!self) {
        ...
        if (err) *err = [OPTLYJSONModelError errorModelIsInvalid];
        return nil;
    }
...
-(OPTLYJSONKeyMapper*)__keyMapper
{
    //get the model key mapper
    return objc_getAssociatedObject(self.class, &kMapperObjectKey);
}

-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(OPTLYJSONKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (OPTLYJSONModelClassProperty* property in [self __properties__]) {
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) {
        ...
        OPTLYJMLog(@"Incoming data was invalid [%@ initWithDictionary:]. Keys missing: %@", self.class, requiredProperties);

        if (err) *err = [OPTLYJSONModelError errorInvalidDataWithMissingKeys:requiredProperties];
        return NO;
    }
...
-(NSString*)__mapString:(NSString*)string withKeyMapper:(OPTLYJSONKeyMapper*)keyMapper
{
    if (keyMapper) {
        //custom mapper
...
-(BOOL)__importDictionary:(NSDictionary*)dict withKeyMapper:(OPTLYJSONKeyMapper*)keyMapper validation:(BOOL)validation error:(NSError**)err
{
    ...
    for (OPTLYJSONModelClassProperty* property in [self __properties__]) {
        ...
        //OPTLYJMLog(@"keyPath: %@", jsonKeyPath);
        ...
        if (isNull2(jsonValue)) {
            ...
            if (err) {
                ...
                OPTLYJSONModelError* dataErr = [OPTLYJSONModelError errorInvalidDataWithMessage:msg];
        ...
        if (isValueOfAllowedType==NO) {
            //type not allowed
            OPTLYJMLog(@"Type %@ is not allowed in JSON.", NSStringFromClass(jsonValueClass));
        ...
        if (property) {
            ...
            // 1) check if property is itself a OPTLYJSONModel
            if ([self __isOPTLYJSONModelSubClass:property.type]) {

                //initialize the property's model, store it
                OPTLYJSONModelError* initErr = nil;
                ...
                if (
                    ...
                    Class sourceClass = [OPTLYJSONValueTransformer classByResolvingClusterClasses:[jsonValue class]];

                    //OPTLYJMLog(@"to type: [%@] from type: [%@] transformer: [%@]", p.type, sourceClass, selectorName);
                    ...
                    if (foundCustomTransformer) {
                        ...
                        if (err) {
                            ...
                            OPTLYJSONModelError* dataErr = [OPTLYJSONModelError errorInvalidDataWithTypeMismatch:msg];
                            *err = [dataErr errorByPrependingKeyPathComponent:property.name];
                        }
                        return NO;
                    }
...
-(BOOL)__isOPTLYJSONModelSubClass:(Class)class
{
// http://stackoverflow.com/questions/19883472/objc-nsobject-issubclassofclass-gives-incorrect-failure
#ifdef UNIT_TESTING
    return [@"OPTLYJSONModel" isEqualToString: NSStringFromClass([class superclass])];
#else
    return [class isSubclassOfClass:OPTLYJSONModelClass];
#endif
}
...
-(NSMutableSet*)__requiredPropertyNames
{
    ...
    if (!classRequiredPropertyNames) {
        ...
        [[self __properties__] enumerateObjectsUsingBlock:^(OPTLYJSONModelClassProperty* p, NSUInteger idx, BOOL *stop) {
...
{
    //OPTLYJMLog(@"Inspect class: %@", [self class]);
    ...
    // inspect inherited properties up to the OPTLYJSONModel class
    while (class != [OPTLYJSONModel class]) {
        //OPTLYJMLog(@"inspecting: %@", NSStringFromClass(class));
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            OPTLYJSONModelClassProperty* p = [[OPTLYJSONModelClassProperty alloc] init];
            ...
            //OPTLYJMLog(@"attr: %@", [NSString stringWithCString:attrs encoding:NSUTF8StringEncoding]);
            ...
            if ([scanner scanString:@"@\"" intoString: &propertyType]) {
                ...
                //OPTLYJMLog(@"type: %@", propertyClassName);
...
#pragma GCC diagnostic pop
            ...
            //the property must be a primitive
                ...
                if (![allowedPrimitiveTypes containsObject:propertyType]) {
                    ...
                    @throw [NSException exceptionWithName:@"OPTLYJSONModelProperty type not allowed"
                                                   reason:[NSString stringWithFormat:@"Property type of %@.%@ is not supported by OPTLYJSONModel.", self.class, p.name]
                                                 userInfo:nil];
                }
            ...
            //few cases where OPTLYJSONModel will ignore properties automatically
            ...
            if (p)
            {
                ...
                {
                    NSString *class = NSStringFromClass([OPTLYJSONValueTransformer classByResolvingClusterClasses:type]);
        ...
        //(will do that until it reaches the root class - OPTLYJSONModel)
...
-(id)__transform:(id)value forProperty:(OPTLYJSONModelClassProperty*)property error:(NSError**)err
{
    Class protocolClass = NSClassFromString(property.protocol);
    if (!protocolClass) {

        //no other protocols on arrays and dictionaries
        //except OPTLYJSONModel classes
        if ([value isKindOfClass:[NSArray class]]) {
            @throw [NSException exceptionWithName:@"Bad property protocol declaration"
                                           reason:[NSString stringWithFormat:@"<%@> is not allowed OPTLYJSONModel property protocol, and not a OPTLYJSONModel class.", property.protocol]
                                         userInfo:nil];
        }
        return value;
    }

    //if the protocol is actually a OPTLYJSONModel class
    if ([self __isOPTLYJSONModelSubClass:protocolClass]) {
        ...
        if ([property.type isSubclassOfClass:[NSArray class]]) {
            ...
            if(![[value class] isSubclassOfClass:[NSArray class]])
            {
                if(err != nil)
                {
                    ...
                    OPTLYJSONModelError* typeErr = [OPTLYJSONModelError errorInvalidDataWithTypeMismatch:mismatch];
            ...
            //one shot conversion
            OPTLYJSONModelError* arrayErr = nil;
...
-(id)__reverseTransform:(id)value forProperty:(OPTLYJSONModelClassProperty*)property
{
    Class protocolClass = NSClassFromString(property.protocol);
    ...
    if ([self __isOPTLYJSONModelSubClass:protocolClass]) {
        ...
        if (property.type == [NSArray class] || property.type == [NSMutableArray class]) {
            ...
            for (NSObject<AbstractOPTLYJSONModelProtocol>* model in (NSArray*)value) {
        ...
        if (property.type == [NSDictionary class] || property.type == [NSMutableDictionary class]) {
            ...
            for (NSString* key in [(NSDictionary*)value allKeys]) {
                id<AbstractOPTLYJSONModelProtocol> model = value[key];
...
- (BOOL)__customSetValue:(id <NSObject>)value forProperty:(OPTLYJSONModelClassProperty *)property
{
    NSString *class = NSStringFromClass([OPTLYJSONValueTransformer classByResolvingClusterClasses:[value class]]);
...
- (BOOL)__customGetValue:(id *)value forProperty:(OPTLYJSONModelClassProperty *)property
{
...
-(NSDictionary*)toDictionaryWithKeys:(NSArray*)propertyNames
{
    ...
    for (OPTLYJSONModelClassProperty* p in properties) {
        ...
        //OPTLYJMLog(@"toDictionary[%@]->[%@] = '%@'", p.name, keyPath, value);
        ...
        if ([value isKindOfClass:OPTLYJSONModelClass]) {

            //recurse models
            value = [(OPTLYJSONModel*)value toDictionary];
            [tempDictionary setValue:value forKeyPath: keyPath];
            ...
            if (YES) {
                ...
                if (foundCustomTransformer) {
                    ...
                    @throw [NSException exceptionWithName:@"Value transformer not found"
                                                   reason:[NSString stringWithFormat:@"[OPTLYJSONValueTransformer %@] not found", selectorName]
                                                 userInfo:nil];
                    return nil;
                }
            }
...
-(NSData*)toJSONDataWithKeys:(NSArray*)propertyNames
{
    ...
    @catch (NSException *exception) {
        //this should not happen in properly design OPTLYJSONModel
        //usually means there was no reverse transformer for a custom property
        OPTLYJMLog(@"EXCEPTION: %@", exception.description);
        return nil;
    }
...
+ (NSMutableDictionary *)dictionaryOfModelsFromDictionary:(NSDictionary *)dictionary error:(NSError * __autoreleasing *)err
{
    ...
    for (NSString *key in dictionary.allKeys)
    {
        ...
        {
            if (err) {
                *err = [OPTLYJSONModelError errorInvalidDataWithTypeMismatch:@"Only dictionaries and arrays are supported"];
            }
            return nil;
        }
    }
...
+(NSMutableArray*)arrayOfDictionariesFromModels:(NSArray*)array
{
    ...
    for (id<AbstractOPTLYJSONModelProtocol> object in array) {
...
+(NSMutableDictionary *)dictionaryOfDictionariesFromModels:(NSDictionary *)dictionary
{
    ...
    for (NSString *key in dictionary.allKeys) {
        id <AbstractOPTLYJSONModelProtocol> object = dictionary[key];
        id obj = [object toDictionary];
        if (!obj) return nil;
        output[key] = obj;
    }
...
-(NSString*)description
{
    ...
    for (OPTLYJSONModelClassProperty *p in [self __properties__]) {
...
+(void)setGlobalKeyMapper:(OPTLYJSONKeyMapper*)globalKeyMapperParam
{
...
-(instancetype)initWithCoder:(NSCoder *)decoder
{
    ...
    OPTLYJSONModelError *error = nil;
    self = [self initWithString:json error:&error];
    if (error) {
        OPTLYJMLog(@"%@",[error localizedDescription]);
    }
    return self;
}
